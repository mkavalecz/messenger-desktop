...
```bash
npm start          # Dev mode: compile + watch + electronmon
...
**Entry point:** `src/main.ts` — initializes app identity, loads settings, creates tray + main window, triggers update check.
...
**Badge:** Title-based unread detection — regex checks if the page title starts with a parenthesised number. Badge clear is debounced 1.5s to avoid flashing.
